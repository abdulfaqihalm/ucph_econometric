...
Today's exercises are based mainly on Chapter 9 in Wooldridge (2018). Some questions from previous chapters may also be included. In addition, the questions you may not have solved last week should be considered prerequisites to this exercise set. Feel free to post questions either by [mail](mailto:cep@ifro.ku.dk) or the discussion forum [here](https://ucph.padlet.org/carlemilpless/questions-ehvgql4yj517uhb3). I will monitor both during the day and will be available for questions.
...
FOr MLR.1 - 3 we don't have to tes them since it's already stated on the problem set.
...
```
From the test above we can see that the F statistic is 2.529, which is statistically insignificant even at 10% level. Hence we can not reject the null hypothesis. Therefore, it might be that our sample is heteroskedasticity.
...
```
From the restricted-F-test above, the p-value is statistically significant even at 1% level. Therefore, we can't reject the null hypothesis which stated that all of the added coefficent are zero.
...
```{r}
hprice1_model5 <- lm(data = hprice1_data,
                    formula = price ~ lotsize + I(lotsize^2) + lotsize:sqrft + sqrft + I(sqrft^2) + sqrft:bdrms + bdrms + I(bdrms^2) + bdrms:lotsize)


hprice1_model6 <- lm(data = hprice1_data,
                    formula = log(price) ~ log(lotsize) + I(log(lotsize)^2) + log(lotsize):log(sqrft) + log(sqrft) + I(log(sqrft)^2) + log(sqrft):bdrms + bdrms + I(bdrms^2) + bdrms:log(lotsize))

resettest(hprice1_model5)
resettest(hprice1_model6)

```
From the above result, the two p-values are both statistically not significant. However, the log() form is smaller which might indicate that it is better model compared to the non-log() form. However, our RESET test suggest that for both non-log and log form, the model specification are not correct. It might be due to the additional squared and cubic fitted. 
